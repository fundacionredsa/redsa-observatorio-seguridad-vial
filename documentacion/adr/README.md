# Architecture Decision Records

Estados: `Aceptada`, `Reemplazada` o `Propuesta`. Nuevas decisiones tecnicas
que cambien datos, arquitectura o interpretacion deben agregar un ADR y no
reescribir retroactivamente la justificacion historica.

- [ADR-001](0001-provincias-derivadas-de-cantones.md): provincias por disolucion.
- [ADR-002](0002-niveles-territoriales-por-zoom.md): cambio de nivel por zoom.
- [ADR-003](0003-edg-y-sppat-usuario-vial.md): roles de EDG y SPPAT.
- [ADR-004](0004-hotspots-getis-ord.md): Gi* y tratamiento de islas.
- [ADR-005](0005-frontend-estatico-github-pages.md): frontend estatico.
- [ADR-006](0006-privacidad-y-separacion-de-repositorios.md): privacidad y repos.
- [ADR-007](0007-experiencia-ciudadana-y-registros-frontend.md): divulgacion
  progresiva, vista nacional y registros frontend.
- [ADR-008](0008-interaccion-territorial-predecible.md): seleccion persistente,
  nivel territorial controlable y frontend modular.
- [ADR-009](0009-responsive-movil-consolidado.md): estrategia responsive unica,
  flujo tactil y cobertura movil permanente.
